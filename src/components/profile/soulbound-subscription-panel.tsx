...
import {
  continueSoulboundPaymentSubscription,
  ...
  pauseSoulboundPaymentSubscription,
  unpauseSoulboundPaymentSubscription,
...
function isNotFoundError(error: unknown) {
  return Number((error as any)?.response?.status ?? 0) === 404;
}
...
export default function SoulboundSubscriptionPanel({
  profileData,
  refetchProfile,
}: Props) {
  const soulboundSubscriptionRoute = endpoints.profile.getSoulboundSubscription;
  const {
  ...
  } = useApiQuery(soulboundSubscriptionRoute, {
    enabled: true,
    retry: false,
  });

  const rawSubscription = useMemo(
    () =>
      (pickDataRoot(paymentSubscriptionResp) ??
        null) as PaymentSubscriptionRecord | null,
    [paymentSubscriptionResp],
  );
  const paymentSubscription = rawSubscription ?? null;
  const paymentSubscriptionId = String(paymentSubscription?._id ?? "").trim();
  const hasLegacySoulboundOnly =
    !paymentSubscriptionId && profileData?.soulbound?.isActive === true;

  const soulboundSubscriptionPaymentsRoute = paymentSubscriptionId
    ? endpoints.profile.getSoulboundSubscriptionPayments(1, 50)
    : "";
  const {
  ...
  } = useApiQuery(soulboundSubscriptionPaymentsRoute, {
    enabled: !!paymentSubscriptionId,
    retry: false,
  });
  ...
  } = useApiQuery(endpoints.profile.getSoulboundSubscriptionAllowance, {
    enabled: !!paymentSubscriptionId,
    retry: false,
  });
  ...
  const subscriptionReadError =
    paymentSubscriptionError && !isNotFoundError(paymentSubscriptionError)
      ? readErrorMessage(
          paymentSubscriptionError,
          "Failed to load soulbound subscription.",
        )
      : null;
  ...
  async function refreshSubscriptionViews() {
    const queries = [
      ...
      queryClient.invalidateQueries({
        queryKey: [endpoints.profile.getSoulboundSubscription],
      }),
    ];
    ...
    if (paymentSubscriptionId) {
      queries.push(
        queryClient.invalidateQueries({
          queryKey: [endpoints.profile.getSoulboundSubscriptionAllowance],
        }),
      );
      queries.push(
        queryClient.invalidateQueries({
          queryKey: [endpoints.profile.getSoulboundSubscriptionPayments(1, 50)],
        }),
      );
    }
  ...
  async function handlePauseRecurring() {
    if (!canPauseRecurring) return;
    setSubscriptionActionBusy("pause");
    setSubscriptionActionError(null);
    try {
      await pauseSoulboundPaymentSubscription();
  ...
  async function handleUnpauseRecurring() {
    if (!canUnpauseRecurring) return;
    setSubscriptionActionBusy("unpause");
    setSubscriptionActionError(null);
    try {
      await unpauseSoulboundPaymentSubscription();
      await refreshSubscriptionViews();
    } catch (error) {
      setSubscriptionActionError(
  ...
  if (!paymentSubscription && !hasLegacySoulboundOnly && !isResolvingSubscription) {
    return (
      <div className="rounded-2xl border border-black/5 bg-white p-6">
        ...
        <h2 className="mt-2 text-2xl font-semibold text-[#111]">
          No recurring soulbound subscription yet
        </h2>
        <p className="mt-2 text-sm text-[#6E6E6E]">
          Once you buy soulbound on the crypto rail, its recurring status,
          payment history, and recovery controls will appear here.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-black/5 bg-white p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-[#6E6E6E]">
            Manage soulbound auto-renew, review the current paid-through
            window, and continue manually if a recurring renewal requires
            action.
          </p>
        </div>
      ...
      <ContinueNowConfirmDialog
        ...
        subjectLabel="soulbound subscription"
      ...
      {showContinueProgress ? (
        ...
        >
          ...
          <p className="mt-2 text-sm">
            {continueStatus === "succeeded"
              ? "The backend confirmed the retry and refreshed your soulbound recurring subscription."
              : continueStatus === "failed" || continueStatus === "canceled"
      ...
      {hasLegacySoulboundOnly && !paymentSubscription ? (
        <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700">
          Your soulbound access is currently running on the legacy one-time
          model until {formatDateTime(profileData?.soulbound?.expireAt)}. The
          recurring management controls here will appear after your next crypto
          soulbound purchase.
        </div>
      ) : null}
